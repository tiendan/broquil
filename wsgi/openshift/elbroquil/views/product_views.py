        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if 'OPENSHIFT_DATA_DIR' in os.environ:
                    detach_dir = os.path.join(
                        os.environ['OPENSHIFT_DATA_DIR'], "temp")
                ...
                if excel_format == models.CAL_ROSSET:
                    ...
                    products, distribution_date, order_limit_date = \
                        parser.parse_standard(book)
...
def confirm_products(request):
    ...
    # If the form is submitted, read the verified information and create the
    # products'
    if request.method == 'POST':
        ...
        with transaction.atomic():
            ...
            models.Product.objects.filter(
                distribution_date=distribution_date,
                category__producer_id=producer_id).delete()
            ...
            for product_info in table_data:
                ...
                integer_demand = "unitat" in comments_text.lower() \
                    or unit_text.lower() != "kg"

                for exceptional_product in [u"carabassa", u"carbassa",
                                            u"síndria", u"meló"]:
                    integer_demand = integer_demand \
                        or exceptional_product in name_text.lower()
                ...
                category, created = models.Category.objects.get_or_create(
                    name=category_text, producer_id=producer_id)
...
def view_products(request, producer_id=''):
    ...
    if producer_id != '':
        products = models.Product.objects.filter(
            distribution_date=None,
            category__producer_id=int(producer_id)) \
    ...
    if len(products) == 0 and producer_id != '':
        products = models.Product.objects.filter(
            Q(distribution_date__gt=libs.get_today()),
            category__producer_id=int(producer_id)) \
            .order_by('category__sort_order', 'id')
