...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            'Comprobando si hay pedidos para enviar a los productores...')
        ...
        for producer in producers:
            ...
            self.stdout.write('Productor: %s...' % producer.company_name)
            self.stdout.write(
                '%d productos pendientes de enviar al productor...' %
                len(products))
            ...
            for product in products:
                total_quantity = models.Order.objects \
                    ...
                    .aggregate(Sum('quantity'))['quantity__sum'] or \
                    Decimal(0)
            ...
            self.stdout.write(
                'El total de los pedidos son: %f euros.' % order_total)

            # If producer has a defined minimum order, and it is not exceeded
            # or simply there was no order
            if (producer.minimum_order and
                order_total < producer.minimum_order) or \
                    order_total == 0:

                if order_total > 0:
                    self.stdout.write(
                        'El total NO LLEGA al minimo (%f)' %
                        producer.minimum_order)

                self.stdout.write(
                    'Enviando el correo de NO PEDIDO al productor...')
                ...
                email_subject = """[BroquilGotic]Aquest cop no arribem """ \
                    """al mínim per a fer la comanda"""
                ...
                if email:
                    self.stdout.write(
                        """NO PEDIDO email plantilla encontrada en el """
                        """base de datos.""")
            ...
            self.stdout.write(
                'Enviando el correo de TOTAL DEL PEDIDO al productor...')
            ...
            if email:
                self.stdout.write(
                    """TOTAL DEL PEDIDO email plantilla encontrada """
                    """en el base de datos.""")
                email_subject = email.full_subject()
                html_content = email.body
