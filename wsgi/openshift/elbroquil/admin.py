...
class AvailabilityInline(admin.TabularInline):
    ...
    verbose_name_plural = _(
        u"Producer availability (in case of limited availability)")
...
class ProducerAdmin(admin.ModelAdmin):
    form = ProducerForm
    fieldsets = (
        (_(u'General Info'), {'fields': [
         'first_name', 'last_name', 'company_name', 'email', 'description']}),
        (_(u'Order Info'), {'fields': ['order_day',
                                       'order_hour',
                                       'minimum_order',
                                       'transportation_cost',
                                       'fixed_products',
                                       'limited_availability']}),
        (_(u'Technical Info'), {'fields': [
...
class CustomUserCreationForm(UserCreationForm):
    ...
    secondary_email = forms.EmailField(
        label=_(u"Alternative email"), required=False)
    secondary_phone = forms.CharField(
        label=_(u"Alt. phone number"), max_length=15, required=False)
...
class EmailTemplateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_(u'Information'), {'classes': ('full-width',),
                             'fields': ('email_code', 'language',)}),
        (_(u'Email'), {'classes': ('full-width',),
                       'fields': ('subject', 'body',)}),
    )
