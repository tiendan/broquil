...
class AvailabilityInline(admin.TabularInline):
    ...
    verbose_name_plural = _("Producer availability (in case of limited availability)")
    verbose_name = _("date")


class ProducerForm(forms.ModelForm):
    class Meta:
        widgets = {"description": CKEditor5Widget(attrs={"class": "django_ckeditor_5"}, config_name="extends")}
...
class ProducerAdmin(admin.ModelAdmin):
    form = ProducerForm
    fieldsets = (
        (_("General Info"), {"fields": ["first_name", "last_name", "company_name", "email", "description"]}),
        (
            _("Order Info"),
            {
                "fields": [
        ...
        (_("Technical Info"), {"fields": ["short_product_explanation", "excel_format", "active"]}),
    )
...
class CategoryAdmin(admin.ModelAdmin):
    ...
    fieldsets = ((None, {"classes": ("wide",), "fields": ("name", "visible_name", "producer", "sort_order")}),)
...
class ProductAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        qs = (
            super(ProductAdmin, self)
            .get_queryset(request)
            .filter(Q(distribution_date__gte=libs.get_today()) | Q(distribution_date=None))
...
class CustomUserCreationForm(UserCreationForm):
    ...
    secondary_email = forms.EmailField(label=_("Alternative email"), required=False)
    secondary_phone = forms.CharField(label=_("Alt. phone number"), max_length=15, required=False)
...
class CustomUserAdminActiveUsers(CustomUserAdmin):
    ...
    def get_queryset(self, request):
        qs = (
            super(CustomUserAdminActiveUsers, self)
            .get_queryset(request)
            .filter(Q(is_active=True) & Q(username__contains="@"))
...
class CustomUserAdminInactiveUsers(CustomUserAdmin):
    ...
    def get_queryset(self, request):
        qs = (
            super(CustomUserAdminInactiveUsers, self)
            .get_queryset(request)
            .filter(~Q(is_active=True) & Q(username__contains="@"))
            .order_by("first_name", "last_name")
        )
...
class CustomUserAdminSystemUsers(CustomUserAdmin):
    ...
    def get_queryset(self, request):
        qs = (
            super(CustomUserAdminSystemUsers, self)
            .get_queryset(request)
...
class EmailTemplateForm(forms.ModelForm):
    class Meta:
        widgets = {"body": CKEditor5Widget(attrs={"class": "django_ckeditor_5"}, config_name="extends")}
...
class DistributionDateAdmin(admin.ModelAdmin):
    ...
    fieldsets = ((None, {"classes": ("wide",), "fields": ("distribution_date", "canceled")}),)

    def get_queryset(self, request):
        qs = (
            super(DistributionDateAdmin, self)
            .get_queryset(request)
