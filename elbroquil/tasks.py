...
def send_order_to_producers():
    logger.info("Comprobando si hay pedidos para enviar a los productores...")
    ...
    for producer in producers:
        ...
        products = models.Product.objects.filter(
            order_limit_date__lt=libs.get_now(), sent_to_producer=False, category__producer=producer
        ).order_by("category__sort_order", "id")

        # If there are no products, continue
        if len(products) == 0:
            logger.debug("No hay productos para este productor.")
            continue

        logger.info("Productor: %s..." % producer.company_name)
        logger.info("%d productos pendientes de enviar al productor..." % len(products))
        ...
        for product in products:
            total_quantity = models.Order.objects.filter(product=product).aggregate(Sum("quantity"))[
                "quantity__sum"
            ] or Decimal(0)
        ...
        logger.info("El total de los pedidos son: %f euros." % order_total)
        ...
        if (producer.minimum_order and order_total < producer.minimum_order) or order_total == 0:
            ...
            if order_total > 0:
                logger.warning("El total NO LLEGA al minimo (%f)" % producer.minimum_order)

            logger.info("Enviando el correo de NO PEDIDO al productor...")
            ...
            if email:
                logger.debug("NO PEDIDO email plantilla encontrada en el base de datos.")
                email_subject = email.full_subject()
                html_content = email.body

            libs.send_email_to_address(email_subject, html_content, [producer.email])

            logger.info("NO PEDIDO email enviado correctamente.")
            # Skip to next producer
            continue

        # Prepare and send the email to the producer
        logger.info("Enviando el correo de TOTAL DEL PEDIDO al productor...")
        ...
        if email:
            logger.debug("TOTAL DEL PEDIDO email plantilla encontrada en el base de datos.")
            email_subject = email.full_subject()
            html_content = email.body
        ...
        for product in products:
            if product.total_quantity > 0:
                order_totals_table += "<tr><td>%s</td><td>%s</td></tr>" % (product.name, str(product.total_quantity))
        ...
        logger.info("TOTAL DEL PEDIDO email enviado correctamente.")
    ...
    if some_orders_processed:
        ...
        if len(products) == 0:
            ...
            if email:
                logger.debug("""PEDIDO ENVIADO AL PRODUCTOR email plantilla """ """encontrada en el base de datos.""")
                email_subject = email.full_subject()
                html_content = email.body
            ...
            logger.info("""PEDIDO ENVIADO AL PRODUCTOR email enviado """ """a %d personas.""" % result[0])
...
def create_offer():
    ...
    if days_until_next_distribution > 7:
        logger.info(
            ...
            """(%s). NO se creara la oferta.""" % (days_until_next_distribution, next_dist_date.strftime("%d/%m/%Y"))
        )
        return
    ...
    logger.info("Intentando descargar el excel del productor principal...")

    excel_file_path = download_cal_rosset_excel()

    if excel_file_path == "":
        logger.error("Problema al descargar el excel del productor principal.")
        return

    logger.info("El excel se ha descargado correctamente.")
    ...
    with transaction.atomic():
        ...
        for product in products:
            ...
            category, _ = models.Category.objects.get_or_create(name=category_text, producer_id=cal_rosset.id)
        ...
        logger.info("Se han importado %d productos del productor principal." % len(products))

    # Update/duplicate available products to create the final offer
    producers = models.Producer.objects.filter(active=True)

    for producer in producers:
        logger.info("Comprobando productor: %s..." % producer.company_name)
        ...
        if producer_next_dist_date != next_dist_date:
            logger.info(
                ...
                """distribucion es: %s, saltando el productor.""" % producer_next_dist_date.strftime("%d/%m/%Y")
            )
            continue

        logger.info("Creando oferta del productor.")
        ...
        products = models.Product.objects.filter(
            category__producer=producer, distribution_date__isnull=True, archived=False
        )

        if products:
            logger.info("El productor tiene %d productos..." % len(products))

            # Delete all products already copied for this week
            models.Product.objects.filter(
                category__producer=producer, distribution_date=next_dist_date, sent_to_producer=False
            ).delete()
            ...
            logger.info("Los productos se han definido correctamente.")

    # Send the reminder email to the active members
    logger.info("""Enviando el correo de informacion sobre la oferta """ """a los miembros...""")
    ...
    if email:
        logger.debug("""OFERTA CREADA email plantilla encontrada en el base """ """de datos.""")
        email_subject = email.full_subject()
        html_content = email.body
    ...
    logger.info("OFERTA CREADA email enviado a %d personas." % result[0])
...
def download_cal_rosset_excel():
    ...
    logger.info(">Connectando a GMAIL")
    ...
    if len(items) == 0:
        logger.warning(f">Ningun email encontrado con el FROM: '{email_from}'.")
        return ""

    logger.info(f">{len(items)} emails encontrados con el FROM: '{email_from}'.")
    ...
    for emailid in items:
        ...
        logger.debug(f"Mail con tema '{mail['Subject']}', con fecha '{mail['Date']}'")
...
def send_sunday_reminder():
    logger.info("Enviando el recordatorio a los miembros...")

    if models.Product.objects.filter(distribution_date=libs.get_next_weekday()).count() == 0:
        logger.info("""No productos en oferta para este miercoles, """ """no se enviara ningun correo...""")
        return
    ...
    if email:
        logger.debug("RECORDATORIO email plantilla encontrada en el base de datos.")
        email_subject = email.full_subject()
        html_content = email.body

    result = libs.send_email_to_active_users(email_subject, html_content)

    logger.info("RECORDATORIO email enviado a %d personas." % result[0])


def send_task_reminder():
    logger.info("Enviando el recordatorio a los responsables de permanencia...")
    ...
    tasks = models.DistributionTask.objects.filter(
        distribution_date__lt=some_days_later, distribution_date__gt=now
    ).prefetch_related("user", "user__extrainfo")

    if tasks.count() == 0:
        logger.info("""No hay permanencias en los proximos dias, """ """no se enviara ningun correo...""")
        return
    ...
    logger.info("RECORDATORIO email enviado a %d personas." % len(to))
