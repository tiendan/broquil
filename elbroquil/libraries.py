...
def calculate_order_summary(request):
    ...
    orders = (
        models.Order.objects.filter(
            user=request.user, archived=False, product__distribution_date=dist_date, status=models.STATUS_NORMAL
        )
    ...
    if len(orders) > 0:
        order_summary = order_summary + "<thead><tr><th>" + _("PRODUCTS") + "</th></tr></thead><tbody>"
        ...
        order_summary = order_summary + "<tbody><tr><td>" + _("No products in cart") + "</td></tr></tbody></table>"

    # Get debt from last weeks (if exists)
    last_debt = (
        models.Debt.objects.filter(user_id=request.user, payment__date__lt=today).order_by("-payment__date").first()
    )
...
def calculate_user_orders(member, dist_date):
    # Get the orders for next distribution date and for this user
    orders = (
        models.Order.objects.filter(user=member, product__distribution_date=dist_date, status=models.STATUS_NORMAL)
        .prefetch_related("product")
        .order_by("-quantity", "product__name")
    )
...
def create_distribution_dates():
    ...
    for i in range(12):
        ...
        existing_date = models.DistributionDate.objects.filter(distribution_date=candidate_date).first()
        ...
        if not existing_date:
            ...
            existing_date = models.DistributionDate.objects.filter(
                distribution_date__in=[day_before, day_after]
            ).first()

        # If it doesn't exist at all, create it
        if not existing_date:
            distribution_date = models.DistributionDate(distribution_date=candidate_date, canceled=False)
            distribution_date.save()
            logger.info(f"Created distribution date: {candidate_date.strftime('%d/%m/%Y')}")
...
def get_next_distribution_date(allow_today=True):
    ...
    next_date = (
        models.DistributionDate.objects.filter(distribution_date__gte=today, canceled=False)
...
def get_producer_next_distribution_date(producer_id, allow_today=True):
    ...
    # Find the next non-canceled distribution date
    next_dates = models.DistributionDate.objects.filter(distribution_date__gte=today, canceled=False).order_by(
        "distribution_date"
    )[:5]
    ...
    if producer.limited_availability:
        for dist_date in next_dates:
            ...
            if models.ProducerAvailableDate.objects.filter(
                available_date=dist_date.distribution_date, producer=producer
            ).exists():
            ...
            if models.ProducerAvailableDate.objects.filter(available_date=day_before, producer=producer).exists():
                return day_before

            if models.ProducerAvailableDate.objects.filter(available_date=day_after, producer=producer).exists():
                return day_after
...
def get_producer_last_distribution_date(producer_id, allow_today=False):
    if allow_today:
        last_distributed_product = (
            models.Product.objects.filter(category__producer_id=producer_id, distribution_date__lte=get_today())
            .order_by("-distribution_date")
            .first()
        )
    else:
        ...
        last_distributed_product = (
            models.Product.objects.filter(category__producer_id=producer_id, distribution_date__lt=distribution_limit)
            .order_by("-distribution_date")
            .first()
        )
...
def get_producer_order_limit_date(producer, next_dist_date):
    ...
    day = datetime.datetime(next_dist_date.year, next_dist_date.month, next_dist_date.day, producer.order_hour)
    ...
    # Convert to UTC
    day = day.astimezone(datetime.timezone.utc)
...
def update_distribution_task_information(year):
    ...
    with transaction.atomic():
        ...
        request = service.events().list(
            calendarId="lt3p1poe2spctu4u9vogokk59g@group.calendar.google.com", timeMin=year_beginning
        )
        ...
        while request is not None:
            ...
            for event in response.get("items", []):
                ...
                for attendee in event.get("attendees", []):
                    ...
                    user = models.User.objects.filter(email=current_email).first()

                    # If not found, try the secondary email too
                    if not user:
                        extra_info = models.ExtraInfo.objects.filter(secondary_email=current_email).first()

                        if extra_info:
                            user = extra_info.user
                        else:
                            message = _("Could not find member with email:") + ""
