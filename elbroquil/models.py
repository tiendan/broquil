...
class Producer(models.Model):
    ...
    order_day = models.SmallIntegerField(_("order day"), choices=DAY_OF_WEEK_CHOICES, default=MONDAY)
    order_hour = models.SmallIntegerField(_("order hour"), validators=[MaxValueValidator(23), MinValueValidator(0)])
    minimum_order = models.DecimalField(_("minimum order"), decimal_places=2, max_digits=7, null=True, blank=True)
    fixed_products = models.BooleanField(_("fixed products"), default=False)
    limited_availability = models.BooleanField(_("limited availability"), default=False)
    excel_format = models.SmallIntegerField(_("excel format"), choices=EXCEL_FORMAT_CHOICES, default=STANDARD)
    default_category_name = models.CharField(_("default category name"), max_length=50, null=True, blank=True)
    active = models.BooleanField(
        _("active"), default=True, help_text=_("<em>(Disable the producer by clearing this option.)</em>")
    )
    transportation_cost = models.DecimalField(_("transportation cost"), decimal_places=2, max_digits=7, default=0)

    short_product_explanation = models.CharField(
        _("short product explanation"),
        max_length=80,
        null=False,
        blank=False,
        default="",
    ...
    description = models.TextField(
        ...
        default="",
        help_text=_(
            "<em>(Long explanation about the producer. If you want to place photos or links to files, a) place them in Google Drive and create public link, b) find their public link from some other website)</em>"
        ),
    )
...
class Product(models.Model):
    ...
    origin = models.CharField(_("origin"), max_length=100, null=True, blank=True)
    ...
    unit = models.CharField(
        _("unit"), max_length=20, help_text=_("<em>(without the &euro; sign or / character.)</em>")
    )
    ...
    total_quantity = models.DecimalField(_("total quantity"), decimal_places=2, max_digits=7, default=0)
    arrived_quantity = models.DecimalField(_("arrived quantity"), decimal_places=2, max_digits=7, default=0)
    ...
    average_rating = models.DecimalField(_("average rating"), decimal_places=1, max_digits=5, default=0)

    new_product = models.BooleanField(
        _("new product"), default=False, help_text=_("<em>(To highlight this product as NEW in the order pages.)</em>")
    )
    sent_to_producer = models.BooleanField(_("sent to producer"), default=False)
...
class Order(models.Model):
    ...
    quantity = models.DecimalField(_("quantity"), decimal_places=2, max_digits=7)
    rating = models.SmallIntegerField(_("rating"), null=True, blank=True, default=None)
    archived = models.BooleanField(_("archived"), default=False)
    status = models.SmallIntegerField(_("status"), choices=STATUS_CHOICES, default=STATUS_NORMAL)
    arrived_quantity = models.DecimalField(_("arrived quantity"), decimal_places=2, max_digits=7)
...
class Payment(models.Model):
    ...
    amount = models.DecimalField(_("quantity"), decimal_places=2, max_digits=7, default=0)
...
class ExtraInfo(models.Model):
    ...
    secondary_email = models.EmailField(_("secondary email"), blank=True, default="")
    phone = models.CharField(_("phone"), max_length=15)
    secondary_phone = models.CharField(_("secondary phone"), max_length=15, blank=True, default="")
...
class DistributionAccountDetail(models.Model):
    ...
    initial_amount = models.DecimalField(_("initial amount"), decimal_places=2, max_digits=7, default=0)
    member_consumed_amount = models.DecimalField(
        _("member consumed amount"), decimal_places=2, max_digits=7, default=0
    )
    total_member_payment_amount = models.DecimalField(
        _("total member payment amount"), decimal_places=2, max_digits=7, default=0
    )

    debt_balance_amount = models.DecimalField(_("debt balance amount"), decimal_places=2, max_digits=7, default=0)
    quarterly_fee_collected_amount = models.DecimalField(
        _("collected fee amount"), decimal_places=2, max_digits=7, default=0
    )
    final_amount = models.DecimalField(_("final amount"), decimal_places=2, max_digits=7, default=0)
    expected_final_amount = models.DecimalField(_("expected final amount"), decimal_places=2, max_digits=7, default=0)
...
class EmailTemplate(models.Model):
    email_code = models.SmallIntegerField(_("email code"), choices=EMAIL_CODE_CHOICES, default="ca")
    language = models.CharField(_("language code"), choices=LANGUAGE_CHOICES, default="ca", max_length=2)
    subject = models.CharField(
        _("subject"), max_length=70, blank=True, default="", help_text=_("<em>(Without [BroquilGotic])</em>")
    )
    body = models.TextField(
        _("body"), blank=True, default="", help_text=_("<em>(Do not remove the [[CONTENT]] parts.)</em>")
    )
    ...
    def __str__(self):
        return str(self.get_email_type()) + " (" + str(self.get_language_name()) + ")"
...
class EmailList(models.Model):
    ...
    email_addresses = models.TextField(_("email addresses"), blank=True, default="")
    cc_task_reminders = models.BooleanField(_("cc task reminders"), default=False)
...
class AccountMovement(models.Model):
    movement_type = models.SmallIntegerField(_("movement type"), choices=MOVEMENT_CHOICES, default=MOVEMENT_FROM_SAFE)
    amount = models.DecimalField(_("amount"), decimal_places=2, max_digits=7, default=0)
    final_amount = models.DecimalField(_("final amount in safe"), decimal_places=2, max_digits=7, default=0)
