...
class pisaFileObject:
    ...
    def __init__(self, uri, basepath=None):
        ...
        if uri.startswith("data:"):
            ...
            if basepath and not parse.urlparse(uri).scheme:
                urlParts = parse.urlparse(basepath)
            else:
                urlParts = parse.urlparse(uri)

            log.debug("URLParts: %r", urlParts)

            if urlParts.scheme == 'file':
                if basepath and uri.startswith('/'):
                    uri = parse.urljoin(basepath, uri[1:])
            ...
            # Drive letters have len==1 but we are looking for things like http:
                ...
                # External data
                if basepath:
                    uri = parse.urljoin(basepath, uri)
