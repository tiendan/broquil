...
class pisaCSSParser(css.CSSParser):
    def parseExternal(self, cssResourceName):
        ...
        if not cssFile:
            return None
        if self.rootPath and parse.urlparse(self.rootPath).scheme:
            self.rootPath = parse.urljoin(self.rootPath, cssResourceName)
        else:
...
class pisaContext(object):
    ...
    def __init__(self, path, debug=0, capacity=-1):
        ...
        parts = parse.urlparse(self.pathDocument)
        if not parts.scheme:
